<?php
  ...
  // Limit "check stock" action to administrators only
  define('CONF_CHECKSTOCK_OVERRIDEN', intval(CONF_CHECKSTOCK && in_array(100, $relaccess)));
      ...
      case 'cat':
          ...
          { //save changes
...

              {
                  ...
                  $q = db_query("insert into ".CATEGORIES_TABLE." (name, parent, products_count, description, picture, ".
                       ...
                       " meta_description, meta_keywords, title, uri ) "." VALUES ('".xToText(trim($_POST["name"]))."', ".
                       (int)$_POST["parent"].",0,'".xEscSQL($_POST["desc"])."','',0, ".(int)$_POST["sort_order"].", ".$allow_products_comparison.", ".$allow_products_search.", ".
                       $show_subcategories_products.", '".xToText(trim($_POST["meta_d"]))."', '".xToText(trim($_POST["meta_k"]))."', '".xToText(trim($_POST["title"]))."', '".xToText(trim($_POST["slug"]))."');");
                  // END Friendly URLs
...

              {
                  ...
                  if(isset($_POST['category_template'])) {
                    ...
                    $q = db_query('SHOW COLUMNS FROM ' . CATEGORIES_TABLE);
                    $tableFields = Array(); // Hash (hash=1842853355)
                    while($row = db_fetch_assoc($q))
                      $tableFields[$row['Field']] = true;
                    if(!isset($tableFields['template']))
                      db_query('ALTER TABLE ' . CATEGORIES_TABLE . ' ADD COLUMN template VARCHAR(128) NOT NULL DEFAULT \'\'');

                    // Update category template
                    $catTemplate_ = mysql_real_escape_string($_POST['category_template']);
                    $sql = "UPDATE " . CATEGORIES_TABLE . "
                            SET template = '$catTemplate_'
...

                  {
                      ...
                      if (category_Moves_To_Its_SubDirectories($_POST["must_delete"], $_POST["parent"]))
                      {
                          ...
                          db_query("update ".CATEGORIES_TABLE." SET name='".xToText(trim($_POST["name"]))."', description='".xEscSQL($_POST["desc"])."', parent=".(int)$_POST["parent"].
                              ...
                              "', title='".xToText(trim($_POST["title"]))."', uri='".xToText(trim($_POST["slug"]))."' WHERE categoryID=".(int)$_POST["must_delete"]);
                      }
              ...
              // BEGIN Parameters For Category
              // Check if module installed
              $q = db_query('SHOW COLUMNS FROM ' . CATEGORIES_TABLE);
              $cols = array();
              while($row = db_fetch_row($q))
                $cols[$row[0]] = true;
              if(!isset($cols['allowed_parameters']))
                db_query('ALTER TABLE ' . CATEGORIES_TABLE . ' ADD COLUMN allowed_parameters text DEFAULT \'\'');
              // hash=1842853355

              // Update category parameters
              $categoryParams = isset($_POST['categoryParams']) ? array_map('intval', $_POST['categoryParams']) : Array();
              $categoryParams_ = implode(',', $categoryParams);
              $sql = "UPDATE " . CATEGORIES_TABLE . " SET allowed_parameters = '$categoryParams_' WHERE categoryID = $categoryID LIMIT 1";
              db_query($sql);
              // END Parameters For Category

              // BEGIN Articles

              // Update category linked articles
              $linkedArticles = isset($_POST['linkedArticles']) ? array_map('intval', $_POST['linkedArticles']) : Array();
              $linkedArticles_ = implode(',', $linkedArticles);
              $sql = "UPDATE " . CATEGORIES_TABLE . " SET articles_linked = '$linkedArticles_' WHERE categoryID = $categoryID LIMIT 1";
              db_query($sql);
...

          {
...

              {
                  ...
                  $handler = opendir('core/tpl/user/'.CONF_DEFAULT_TEMPLATE.'/');
                  while($file = readdir($handler))
                    if(preg_match('|^category\-([\w_\-]+)\.tpl$|', $file, $matches))
                      $categoryTemplates[] = $file;
...
[<a href="JavaScript:hfu_translit_inputs('name','slug','<?php echo FU_WHITESPACE ?>')" style="text-decoration:underline;">транслит</a>]
</td></tr>
<!-- END Friendly URLs -->
...
<?php
              ...
              // Get articles linked to current category
              $sql = '
                SELECT articles_linked
                FROM '.CATEGORIES_TABLE.'
                WHERE categoryID=\''.intval($_GET["categoryID"]).'\'
                LIMIT 1';
              $q = db_query($sql);
              ...
              $articlesList = Array();
              while($row = db_fetch_row($q)) {
                $row['isLinked'] = in_array($row['AID'], $linkedArticles);
                $articlesList[] = $row;
              }
...
<table class="adn">
<tr class="lineb">
    <td align="left">Привязанные статьи (<?php echo count($linkedArticles).' из '.count($articlesList) ?>)</td>
  </tr>
  <tr>
    <td align="left">
      <div<?php echo (count($articlesList) > 10) ? ' style="height:150px;overflow-y:scroll;"' : '' ?>><?php
...
?>
  <tr>
    <td><input type="checkbox" name="linkedArticles[]" id="linkedArticles_<?php echo $aid ?>" value="<?php echo $aid ?>"<?php echo $article['isLinked'] ? ' checked' : '' ?>></td>
    <td><label for="linkedArticles_<?php echo $aid ?>"><?php echo htmlspecialchars($article['title']) ?></label></td>
  </tr>
<?php
      endforeach;
    echo '
...
';
    ...
    echo '<a href="'.ADMIN_FILE.'?dpt=modules&sub=articles" title="Управление статьями">нет статей</a>';
?>
      </div>
    </td>
  </tr>
...
?></label></td></tr>
<?php if(isset($categoryTemplates)) : ?>
<!-- BEGIN Category Custom Templates -->
<tr><td align="left" valign="middle" colspan="2">
  <label for="select_category_template">Шаблон:</label>
  <select name="category_template" id="select_category_template">
    <option value="">По умолчанию</option>
    <?php
      foreach($categoryTemplates as $categoryTemplate) {
        $tplName = htmlspecialchars($categoryTemplate);
        $selected = ($categoryTemplateCurrent == $categoryTemplate) ? ' selected' : '';
        echo "<option value=\"$tplName\"$selected>$tplName</option>";
      }
    ?>
  </select>
</td></tr>
...
<!-- BEGIN Parameters For Category -->
<script>!window.jQuery && document.write('<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.4/jquery.min.js"><\/script>');</script>
<tr class="lineb">
  <td align="left"><a href="JavaScript:void(0);" onclick="$('#paramsList').toggle();" style="text-decoration:underline;color:#4E6592;">Параметры товаров</a></td>
</tr>
<tr class="lins" id="paramsList" style="display:none;">
  <td align="left">
    <input type="checkbox" id="categoryParams_checkAll" style="position:relative;top:3px;" onclick="$('input.categoryParams').each(function() { $(this).attr('checked', $(this).is(':checked') ? '' : 'checked' );}).trigger('change');"><label for="categoryParams_checkAll"><i>инвертировать выбор</i></label><br>
<?php
  if(count($options)) {
  
    // Get options selected
    $categoryID_ = intval(@$_GET['categoryID']);
    $sql = "SELECT allowed_parameters
              FROM " . CATEGORIES_TABLE . "
              WHERE categoryID = $categoryID_
              LIMIT 1";
    $q = db_query($sql);
    $paramsSelected = Array(); // Hash array
    if($row = db_fetch_row($q)) {
      $paramsSelectedArray = explode(',', $row[0]);
      foreach($paramsSelectedArray as $optionId)
        $paramsSelected[$optionId] = true;
    }

    foreach($options as $option) {
      $checkedStr = isset($paramsSelected[$option['optionID']]) ? ' checked="checked"' : '';
      echo "<input type=\"checkbox\" name=\"categoryParams[]\" id=\"categoryParams_{$option['optionID']}\" value=\"{$option['optionID']}\"$checkedStr class=\"categoryParams\" style=\"position:relative;top:3px;\"><label for=\"categoryParams_{$option['optionID']}\">{$option['name']}</label><br>\n";
    }
      ...
      case 'prod':
          ...
          if (isset($_POST["save_product"]) || isset($_POST["save_product_for_file"]))
          {
              ...
              if ($_POST["save_product"] == 1 || $_POST["save_product_for_file"] == 1)
              {
                  ...
                  {
                      // BEGIN Friendly URLs
                      $productID = AddProduct($_POST["categoryID"], $_POST["name"], $_POST["price"], $_POST["productivity"], $_POST["foto_keywords"],
                  ...
                  {
                      // BEGIN Friendly URLs
                      UpdateProduct($productID, $_POST["categoryID"], $_POST["name"], $_POST["price"], $_POST["productivity"], $_POST["foto_keywords"],
          ...
          if (@$_POST["save_spwc"] == 1 && @$_POST["AddProductAndOpenConfigurator"] == 0)
          {
              ...
              UpdateProduct($productID, $_POST["categoryID"], $_POST["name"], $_POST["price"], $_POST["productivity"], $_POST["foto_keywords"], $_POST["description"],
...
<!-- REQUIRE: JQuery -->
<script type="text/javascript" src="data/admin/img_uploader/swfupload.js"></script>
<script type="text/javascript" src="data/admin/img_uploader/fileprogress.js"></script>
<script type="text/javascript" src="data/admin/img_uploader/swfupload.queue.js"></script>
<script type="text/javascript" src="data/admin/img_uploader/ImageUploader.js"></script>
<link rel="stylesheet" href="data/admin/img_uploader/ImageUploader.css" type="text/css">
<!-- END Image Uploader -->
<title><?php
        ...
        <?php
          ...
          if (isset($_POST["AddProductAndOpenConfigurator"]))
          {
              if ($_POST["AddProductAndOpenConfigurator"] == 1)
              {
                  ...
                  // BEGIN Friendly URLs
                  $productID = AddProduct($_POST["categoryID"], $_POST["name"], $_POST["price"], $_POST["productivity"], $_POST["foto_keywords"], $_POST["description"],
          ...
          if ($_GET["productID"] != 0)
          {
              ...
              $product["foto_keywords"] = html_spchars($product["foto_keywords"]);
...
?>: <input type="text" name="name" value="<?php
          echo $product["name"];
?>" style="width: 590px;" class="textp" onblur="JavaScript:if(!$('[name=slug]').attr('value')) hfu_translit_inputs('name','slug','<?php echo FU_WHITESPACE ?>');"></td></tr>
<tr class="lins"><td align="left"><?php
          echo ADMIN_PRODUCT_TITLE_PAGE;
?>: <input type="text" name="title" value="<?php
          echo $product["title"];
?>" style="width: 590px;" class="textp">
...
<br><br><input type=checkbox name='free_shipping'
                <?php
...
?></td>
<td align="left" width="25%">Произвольный параметр</td>
<?php
...
<td align="left"><input type="text" name="weight" value="<?php
          echo $product["weight"];
?>" style="width: 100px;" class="textp"></td>
<td align="left"><input type="text" name="productivity" value="<?php
          echo $product["Productivity"];
?>" style="width: 100px;" class="textp"></td>
<?php
          if (CONF_CHECKSTOCK == 1)
          {
...
<tr>
<td><textarea name="foto_keywords" id="keys" class="admin"><?php
          echo $product["foto_keywords"];
?></textarea></td>
</tr></table>    
<table class="adn"><tr><td class="separ"><img src="data/admin/pixel.gif" alt="" class="sep"></td></tr><tr><td class="se5"></td></tr></table>
...
<!-- BEGIN Image Uploader -->
<script type="text/javascript">
  var ImageUploaderParams = { "SID": "<?php echo session_id(); ?>", "productID": "<?php echo intval(@$_GET['productID']); ?>" };
</script>
...
<td colspan="5" align="left" valign="middle">
	<div class="fieldset flash" id="fsUploadProgress">
		<span class="legend">Загрузка</span>
	</div>
	<div><span id="divStatusCounter">0</span> изображений загружено</div>
	<div id="imgUploadButtonPanel">
		<span id="spanButtonPlaceHolder"></span>
		<input id="btnCancel" type="button" value="Остановить загрузку" onclick="swfu.cancelQueue();" disabled="disabled" />
	</div>
                                </td>
<!-- END Image Uploader -->
...
<?php
          ...
          // BEGIN Parameters For Category
          // Get all parent categories for the product
          $useParams = Array();
          if($currentCatId = $product["categoryID"]) {
            $productCats = Array();
            while($currentCatId && isset($fc[$currentCatId])) {
              $productCats[] = $currentCatId;
              $currentCatId = $fc[$currentCatId]['parent'];
            }
            
            // Get allowed parameters for parent categories
            $productCats_ = implode(',', $productCats);
            $sql = "SELECT categoryID, allowed_parameters FROM " . CATEGORIES_TABLE . " WHERE categoryID IN ($productCats_)";
            $q = db_query($sql);
            $productCatsParams = Array(); // Hash
            while($row = db_fetch_assoc($q))
              $productCatsParams[$row['categoryID']] = $row['allowed_parameters'];
            
            // Get closest category with params set
            $i = 0;
            while(!count($useParams) && isset($productCats[$i])) {
              if($paramsStr = $productCatsParams[$productCats[$i]])
                $useParams = explode(',', $paramsStr);
              $i++;
            }
          ...
          if (count($options) >= 1)
          {
              ...
              {
                  // BEGIN Parameters For Category
                  // Apply params whitelist
                  if(count($useParams) && (false === array_search($option['option_row']['optionID'], $useParams)))
                    continue;
                  // END Parameters For Category
