...
#####################################
        ...
        if ( CONF_BACKEND_SAFEMODE != 1 && (!isset($_SESSION["log"]) || !in_array(23,$relaccess))) //unauthorized
        {
                        ...
                        function _exportProducts($f, $rate, $export_product_name, $categories_select )
                        {
                                ...
                                // BEGIN Human friendly URL's
                                $sql = "select productID, name, Price, categoryID, default_picture".$dsc_q.", in_stock, uri, uri_opt_val from ".PRODUCTS_TABLE." where enabled=1".$clause;
                                // END Human friendly URL's
                                $q = db_query($sql);
                                ...
                                while ($product = db_fetch_row($q))
                                {
                                        ...
                                        //$url .= strpos($url, '?') ? '&amp;from=ya' : '/from/ya';
                                        fputs( $f, "                                                <url>".$url."</url>\n" );
                                        // END Human friendly URL's
