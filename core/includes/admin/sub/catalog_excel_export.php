...
#####################################
        ...
        function _exportProducts($categoryID, &$f, $delimiter) //writes all products inside a single category to a CSV file
        {
                ...
                //products
                $q1 = db_query("select sort_order, product_code, name, description, brief_description, Price, Productivity, foto_keywords, list_price, in_stock, items_sold, meta_keywords,
                ...
                while ($row1 = db_fetch_row($q1))
                {
                        ...
                        $lines = array($row1["sort_order"],xHtmlSpecialCharsDecode($row1["product_code"]),xHtmlSpecialCharsDecode($row1["name"]),xHtmlSpecialCharsDecode($row1["title"]),$row1["description"],$row1["brief_description"],$row1["Price"],$row1["Productivity"],$row1["foto_keywords"],
        ...
        if ( CONF_BACKEND_SAFEMODE != 1 && (!isset($_SESSION["log"]) || !in_array(6,$relaccess))) //unauthorized
        {
                ...
                if (isset($_POST["excel_export"])) //export products
                {
                        ...
                        ADMIN_PRODUCT_BRIEF_DESC,ADMIN_PRODUCT_PRICE,ADMIN_PRODUCT_PRODUCTIVITY,ADMIN_PRODUCT_FOTO_KEYWORDS,ADMIN_PRODUCT_LISTPRICE,ADMIN_PRODUCT_INSTOCK,ADMIN_PRODUCT_SOLD,
