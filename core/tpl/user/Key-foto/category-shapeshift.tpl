{* -> ShapeShift - скрипт, распределяющий контейнеры (в нашем случае - картинки) равномерно по площади страницы
   -> Есть проблема в браузере Chrome и его аналогах на мобильных платформах - контейнеры склеиваются в кучу
*}
...
{* FancyBox - скрипт всплывающего контейнера с контентом (картинки, таблицы, шаблоны, видео *}
...
$(function() {
    ...
    $('#container-ss').shapeshift({
        ...
        paddingY: 0,
        enableDrag: true
    }); // Устанавливаем динамическую сетку Shapeshift
    
    // Скрипт всплывающей подсказки к картинке
    var duration = 200;
    ...
    $.each($.browser, function(i, val) {
        $("<div>" + i + " : <span>" + val + "</span>").appendTo( $('#cl') );
    });
});
</script>
{/literal}
...
{/if}
<div style="margin: 10px"><a id="cl"></a></div>
{* Вывод товаров *}
<div id="container-ss">
{section name=u loop=$products_to_show}
    <div>
        {include file="product_brief-ss.tpl" product_info=$products_to_show[u]}
    </div>
{/section}
</div>
