{* шаблон блока новых товаров с горизонтальной прокруткой *}
...
<td><img src="data/{$smarty.const.TPL}/pixel.gif" class="fixhblock_{$blocknum}" alt=""><br><a href="{$new_products[i]|@fu_make_url}"><img src="data/{$new_products[i].filename}" alt="{$new_products[i].name}"></a><div class="fil"></div><b class="price">{if $new_products[i].cena <= 0}{$smarty.const.STRING_NOPRODUCT_IN}{else}{$new_products[i].Price}{/if}</b><div class="fil"></div><a href="{$new_products[i]|@fu_make_url}">{$new_products[i].name}</a></td>
{/section}
{section name=i loop=$new_products}
<td><img src="data/{$smarty.const.TPL}/pixel.gif" class="fixhblock_{$blocknum}" alt=""><br><a href="{$new_products[i]|@fu_make_url}"><img src="data/{$new_products[i].filename}" alt="{$new_products[i].name}"></a><div class="fil"></div><b class="price">{if $new_products[i].cena <= 0}{$smarty.const.STRING_NOPRODUCT_IN}{else}{$new_products[i].Price}{/if}</b><div class="fil"></div><a href="{$new_products[i]|@fu_make_url}">{$new_products[i].name}</a></td>
{/section}
...
<!--
var hBlocks_{$blocknum} = 2; //  оличество боксов с информацией.
...
{literal}{{/literal}
        ...
        hPos_{$blocknum} = -3; // ѕоправка на тормознутось браузеров
        {literal}}{/literal}
{literal}}{/literal}
