...
{
// BEGIN Patch Optimize 02
  static $optHash;
  if(!isset($optHash)) $optHash = Array();

  // If option not cached
  if(!isset($optHash[$settings_constant_name])) {
    // Get option from database
        $q = db_query("select settings_value from ".SETTINGS_TABLE.
                " where settings_constant_name='".xEscSQL($settings_constant_name)."'" );
    $optHash[$settings_constant_name]
      = ($row = db_fetch_row($q))
        ? $row["settings_value"]
        : null;
  ...
  return $optHash[$settings_constant_name];
...
function settingCONF_DEFAULT_SORT_ORDER()
{
        ...
        $sortlist[] = array("title"=>STRING_SET_ID4, "value"=>"Productivity ASC");
        $sortlist[] = array("title"=>STRING_SET_ID5, "value"=>"Productivity DESC");
