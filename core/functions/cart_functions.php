...
/**
 * Получение содержимого корзины - из базы (при авторизованном пользователе) или из сессионных переменных
 * 
 * @return array
 */
function cartGetCartContent()
{
        ...
        if (isset($_SESSION["log"])) //get cart content from the database
        {
                ...
                while ($cart_item = db_fetch_row($q))
                {
                        ...
                        $q_products = db_query("select name, Price, productID, default_picture, min_order_amount, shipping_freight, free_shipping, product_code, categoryID, uri, uri_opt_val FROM ".
                                        PRODUCTS_TABLE." WHERE productID=".(int)$shopping_cart_item["productID"]); 
                        // BEGIN User Friendly URLs
                        if ( $product = db_fetch_row($q_products) ) // Получаем массив с одним товаром $product
                        {
                                // BEGIN картинки к товарам в корзине
                                // Получаем картинки товара
                                _setPictures($product); // $product['picture'], $product['thumbnail'], $product['big_picture']
                                // END картинки к товарам в корзине
                ...
                if ( isset($_SESSION["gids"]) )
                        for ($j=0; $j<count($_SESSION["gids"]); $j++)
                        {
                                if ($_SESSION["gids"][$j])
                                {
                                        ...
                                        // BEGIN User Friendly URLs
                                        $q = db_query("select name, Price, productID, default_picture, shipping_freight, free_shipping, product_code, categoryID, uri, uri_opt_val FROM ".
