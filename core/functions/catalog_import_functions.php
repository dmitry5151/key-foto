...
function _isCategory($row, $dbc)
{
        ...
        if ( _columnIsSet($row, $dbc, "Price") )
                return false;
        if ( _columnIsSet($row, $dbc, "Productivity") )
                return false;
        if ( _columnIsSet($row, $dbc, "foto_keywords") )
                return false;
                                ...
                                <tr class=\"lnst\">
                                        ...
                                        <td>
                                                <select name=\"db_association_".$j."\">
                                                        ...
                                                        <option value=\"Productivity\"".mark_as_selected($data[$i][$j],ADMIN_PRODUCT_PRODUCTIVITY).">".ADMIN_PRODUCT_PRODUCTIVITY."</option>
                                                        <option value=\"foto_keywords\"".mark_as_selected($data[$i][$j],ADMIN_PRODUCT_FOTO_KEYWORDS).">".ADMIN_PRODUCT_FOTO_KEYWORDS."</option>
...
function _createBackwards( $db_association )
{
        $dbc = array(
                ...
                "Price"                       => "not defined",
                "Productivity"                => "not defined",
                "foto_keywords"                => "not defined",
        ...
        foreach( $db_association as $i => $value )
        {
                ...
                else if ($value == "Productivity")                              $dbc["Productivity"] = $i;
                else if ($value == "foto_keywords")                              $dbc["foto_keywords"] = $i;
                        ...
                        $updated_extra_option, $currentCategoryID  )
{
        ...
        if ( $rowdb ) //update product info
        {
                ...
                if ( strcmp($dbc["Productivity"], "not defined") )
                        $Productivity = $row[ $dbc["Productivity"] ];
                else $Productivity = $rowdb["Productivity"];
                if ( strcmp($dbc["foto_keywords"], "not defined") )
                        $foto_keywords = $row[ $dbc["foto_keywords"] ];
                else $foto_keywords = $rowdb["foto_keywords"];
        ...
        else // add new product
        {
                ...
                $productID = AddProduct(
                                $currentCategoryID, $row[ $dbc["name"] ], $Price, $Productivity, $row[ $dbc["foto_keywords"] ], $row[ $dbc["description"] ],
                            $in_stock,
