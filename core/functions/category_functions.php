...
// BEGIN Patch Optimize 03 (by http://shopcms-moduli.com/ soulmare@gmail.com)
// Call update_psCount for 1st level parent
function update_psCount_parent($catId=0) {
  if(!$catId || (1 == $catId))
    return update_psCount(1);
  else {
    global $fc, $mc;
    
    $parent = $catId;
    while(isset($mc[$parent]) && ($mc[$parent] > 1))
      $parent = $mc[$parent];

    if($parent < 1)
      $parent = 1;

    return update_psCount($parent);
  }
}

function update_psCount($parent)
{
//if(1 == $parent) return; // deny global re-calculation
global $fc, $mc;
...
// BEGIN Patch Optimize 01 (by http://shopcms-moduli.com/ soulmare@gmail.com)
  ...
  if(count($r)) {
    $subcatsSet = catGetSubCategories($r);
    $r = array_merge($r, $subcatsSet);
        }
