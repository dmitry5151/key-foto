...
function auxpgGetAuxPage( $aux_page_ID )
{
        $q = db_query("select aux_page_ID, aux_page_name, aux_page_text, aux_page_text_type, ".
                 " meta_keywords, meta_description, title, uri from ".AUX_PAGES_TABLE." where aux_page_ID=".(int)$aux_page_ID);
...
function auxpgUpdateAuxPage(    $aux_page_ID, $aux_page_name,
                                ...
                                $meta_keywords, $meta_description, $aux_page_title, $uri  )
{
                                ...
                                $meta_keywords, $meta_description, $aux_page_title, $uri)
{
        db_query( "insert into ".AUX_PAGES_TABLE.
                " ( aux_page_name, aux_page_text, aux_page_text_type, meta_keywords, meta_description, title, uri )  ".
                " values( '".xToText($aux_page_name)."', '".xEscSQL($aux_page_text)."', ".(int)$aux_page_text_type.", ".
                " '".xToText($meta_keywords)."', '".xToText($meta_description)."', '".xToText($aux_page_title)."', '".xToText($uri)."' ) " );
}
